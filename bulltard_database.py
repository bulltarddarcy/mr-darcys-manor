...
def run_strike_zones_app(df):
    ...
    # Restored fallback manual input exactly as in original file
    if spot is None: 
...
def run_rsi_divergences_app():
    ...
    # Path setup based on your generation script
    # Finds latest HTML file in dashboards/divergences
    ...
    if not files:
        ...
        st.info("Ensure the `divergence_make_dashboard.py` script has run.")
        return
    ...
    try:
        ...
        st.caption(f"Showing report: {file_name}")
...
if st.session_state["authentication_status"]:
    ...
    try:
        ...
        # Calculate most recent trade date for the sidebar

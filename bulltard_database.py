    ...
    if data_option:
        try:
            ...
            if csv_buffer and csv_buffer != "HTML_ERROR":
                ...
                # Determine max date in the dataset for highlighting "recent" signals
                latest_date_str = None
                try:
                    # Mimic prepare_data column cleaning to robustly find the date column
                    temp_cols = [c.strip().replace(' ', '').replace('-', '').upper() for c in master.columns]
                    date_c_name = next((master.columns[i] for i, c in enumerate(temp_cols) if 'DATE' in c), None)
                    if date_c_name:
                        latest_date_str = pd.to_datetime(master[date_c_name]).max().strftime('%Y-%m-%d')
                except:
                    pass
                ...
                if raw_results:
                    ...
                    for tf in ['Daily', 'Weekly']:
                        # No st.divider() here to avoid grey lines
                        for s_type, emoji in [('Bullish', '🟢'), ('Bearish', '🔴')]:
                            ...
                            if not tbl_df.empty:
                                ...
                                for row in tbl_df.itertuples():
                                    # Highlight Signal Date if it matches the latest trading date
                                    sig_date = row._6
                                    sd_style = 'text-align:center'
                                    if latest_date_str and sig_date == latest_date_str:
                                        sd_style += '; background-color: #fff9c4; font-weight:bold; color: #555; border-radius: 4px;'
                                    ...
                                    row_html = [
                                        ...
                                        f'<td style="{sd_style}">{sig_date}</td>', 
...
def run_rsi_percentiles_app():
    ...
    # User inputs below loading status
    # But first we need the pills
    ...
    # Inputs below the status bar area
    c1, c2 = st.columns(2)
    with c1:

...
def run_rsi_scanner_app(df_global):
    ...
    # --- Helper: Finance Formatting (Negative in Parens) ---
    def fmt_finance(val):
        if pd.isna(val): return ""
        # Handle cases where val might be a string already
        if isinstance(val, str): return val
        
        # Determine strict sign
    ...
    with tab_single:
        with st.expander("ℹ️ Page Notes: Methodology"):
            ...
            * **Year Total (Right Column):** The **SUM** of returns for that year. Represents total annual performance.
            * **Month Average (Bottom Row):** The **AVERAGE** return for that specific month across all history. Represents seasonal expectancy.
            """)
        ...
        if hist_df.empty:
            ...
            if hist_filtered.empty:
                ...
                with col_chart1:
                    ...
                    # Add Formatted Label column for tooltips/text
                    ...
                    st.altair_chart(
                        (lines + labels).properties(height=350).configure_axis(labelFontSize=11, titleFontSize=13), 
                        use_container_width=True
                    )
                ...
                with col_chart2:
                    ...
                    bars = base.mark_bar().encode(
                        ...
                        color=alt.condition(
                            alt.datum.Value > 0,
                            alt.value("#2e7d32"),  # Green
                            alt.value("#d32f2f")   # Red
                        )
                    )

                    # Text Labels "Above" the bar (physically higher Y for negative bars)
                    # dy = -10 moves text "Up" on the screen. 
                    # For positive bar (top is 5), text goes to 5+ (Above).
                    # For negative bar (bottom is -5), text goes to -5+ (Inside/Above).
                    text = base.mark_text(
                        dy=-10,
                        fontSize=11, 
                        fontWeight='bold',
                ...
                # 2. Month Average (Mean)
                # Calculate mean of columns (ignoring the 'Year Total' column we just added, or include it? Usually average year total is nice too)
                # Let's calculate mean of just the months first
                avg_row = full_pivot[month_names].mean(axis=0)
                
                # Calculate mean of the Year Totals
                avg_total = full_pivot["Year Total"].mean()
                
                avg_row["Year Total"] = avg_total
            ...
            if not ticker_map:
                ...
                def calc_forward_returns(ticker_sym):
                    try:
                        ...
                        matches = d_df[(d_df['DOY'] >= target_doy - 7) & (d_df['DOY'] <= target_doy + 7)].copy()
        ...
        if hist_df.empty:
            ...
            if hist_filtered.empty:
                ...
                with col_chart2:
                    ...
                    # Updated to gentler colors
                    bars = base.mark_bar().encode(
                        y=alt.Y('Value', title='Return (%)'),
                        xOffset='Type',
                        color=alt.condition(
                            alt.datum.Value > 0,
                            alt.value("#71d28a"),  # Gentle Green
                            alt.value("#f29ca0")   # Gentle Red
                        )
                    )

...
def run_seasonality_app(df_global):
        ...
        if hist_df.empty:
            ...
            if hist_filtered.empty:
                ...
                labels = line_base.mark_text(
                    align='center', baseline='bottom', dy=-10, fontSize=16, fontWeight='bold' # INCREASED FONT
                ).encode(text=alt.Text('Value', format='.1f'))
                
                st.altair_chart(
                    (lines + labels).properties(height=350)
                    .configure_axis(labelFontSize=14, titleFontSize=16) # INCREASED AXIS FONT
                ...
                pos_labels = base.transform_filter(alt.datum.Value >= 0).mark_text(
                    dy=-10, fontSize=16, fontWeight='bold', color='black' # INCREASED FONT
                ).encode(
                    y=alt.Y('Value'), xOffset='Type', text=alt.Text('Value', format='.1f')
                )

                neg_labels = base.transform_filter(alt.datum.Value < 0).mark_text(
                    dy=15, fontSize=16, fontWeight='bold', color='black' # INCREASED FONT
                ).encode(
                    y=alt.Y('Value'), xOffset='Type', text=alt.Text('Value', format='.1f')
                )
    ...
    def fetch_history_optimized(ticker_sym, t_map):
        # 1. Try Parquet (Fastest)
        # keys are UPPERCASE in t_map, so we check for _PARQUET
        pq_key = f"{ticker_sym}_PARQUET"
    ...
    with tab_scan:
        ...
        if start_scan:
            ...
            if not ticker_map:
                ...
                # FIX 1: Filter out the PARQUET keys (UPPERCASE) so we don't double count
                ...
                if not results:
                    ...
                    # FIX 2: Custom styling function (No matplotlib)
                    def highlight_ev(val):
                        if pd.isna(val): return ""
            ...
            if not ticker_map:
                ...
                with ThreadPoolExecutor(max_workers=20) as executor: 
                    ...
                    for future in as_completed(futures):
                        ...
                        completed += 1
                        if completed % 5 == 0:
                            progress_bar.progress(completed / len(valid_tickers))
                ...
                if not results:
                    ...
                    st.dataframe(
                        top_30[['Ticker', '30d_EV', '30d_WR', 'N']].style.format({
                            '30d_EV': '{:+.1f}%',
                            '30d_WR': '{:.1f}%'
                        }).background_gradient(subset=['30d_EV'], cmap='RdYlGn'),
                        use_container_width=True,
                        hide_index=True
                    )
                    ...
                    with c_scan1:
                        ...
                        st.dataframe(
                            top_60[['Ticker', '60d_EV', '60d_WR', 'N']].style.format({
                                '60d_EV': '{:+.1f}%',
                                '60d_WR': '{:.1f}%'
                            }).background_gradient(subset=['60d_EV'], cmap='RdYlGn'),
                            use_container_width=True,
                            hide_index=True
                        )
                    ...
                    with c_scan2:
                        ...
                        st.dataframe(
                            top_90[['Ticker', '90d_EV', '90d_WR', 'N']].style.format({
                                '90d_EV': '{:+.1f}%',
                                '90d_WR': '{:.1f}%'
                            }).background_gradient(subset=['90d_EV'], cmap='RdYlGn'),
                            use_container_width=True,
                            hide_index=True
                        )

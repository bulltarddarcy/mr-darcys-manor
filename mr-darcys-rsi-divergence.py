...
# --- Streamlit UI Setup ---
st.set_page_config(page_title="RSI Darcy Scanner", layout="wide")
st.title("📈 RSI Price Darcy Scanner")

# Dataset Selection Sidebar
data_option = st.sidebar.selectbox(
    "Select Dataset to Analyze",
    ("Darcy Data", "S&P 500 Data")
)

# Accessing secrets
try:
    if data_option == "Darcy Data":
        # Updated secret key to URL_DARCY
        raw_url = st.secrets["URL_DARCY"]
    else:
        raw_url = st.secrets["URL_SP500"]
    
    DATA_URL = get_gdrive_download_url(raw_url)
except KeyError:
    # Updated error message to reflect the new secret key requirement
    st.error("Secrets not found. Please ensure URL_DARCY and URL_SP500 are set in Streamlit Secrets.")
    st.stop()
